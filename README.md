...
> Curated agent memory patterns and implementations
...
|---------|-------------|--------|
| Core — Curated foundation for memory workflows | Core foundation — the primary abstraction this repo builds on | [docs](#) |
| Execution — task decomposition and memory routing | Execution layer — how tasks get decomposed and routed | [docs](#) |
...
```bash
# Quick start
python3 main.py --task "memory task here"

# Power user shortcut
python3 main.py --memory --fast
...
Use Awesome-Agent-Memory-main to automate curated agent memory patterns and implementations. Perfect for agencies, freelancers, and AI-first teams running memory workflows at scale.
...
```bash
# Add to CLAUDE.md
echo "Auto-activate: memory, agent" >> ~/.claude/CLAUDE.md

# Or load skill directly
/load-skill Awesome-Agent-Memory-main
```
...
{
  "hooks": {
    "PostToolUse": [{"matcher": "memory", "command": "python3 ~/repos/Awesome-Agent-Memory-main/main.py"}]
  }
}
```
...
```bash
# Fork and clone
gh repo fork hmzainjamil/Awesome-Agent-Memory-main --clone
cd Awesome-Agent-Memory-main
